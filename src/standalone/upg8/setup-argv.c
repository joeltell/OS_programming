...
/*
  ...
  gcc -m32 -Wall -Wextra -std=gnu99 -g ../../lib/string.c ../../lib/debug.c setup-argv.c
...
{
  //en rad = 4 character (4 bytes)
  //för varje rad skall det finnas en char* till den raden (4bytes)(4*argc)
  //en rad för argv (4bytes)
  // en rad för argc(int) (4bytes)
  //en rad för return address (4bytes)
  //en rad för a./aout and I ooop
...
void* setup_main_stack(const char* command_line, void* stack_top)
{
  ...
  printf("# stack_top = %0*lx\n", 2*S, (unsigned long)stack_top);
  //printf("stack_top : %d \n",(int)stack_top);
  esp = stack_top-(total_size);


  /* setup return address and argument count */
  esp->ret = stack_top-total_size;
  esp->argc = argc;
  /* calculate where in the memory the argv array starts */
  esp->argv = stack_top-(total_size)+8;//stack_top-line_size-4;

  printf("# argv = %0*lx\n", 2*S, (unsigned long)esp->argv);
  /* calculate where in the memory the words is stored */
  cmd_line_on_stack = stack_top - line_size;
  printf("# cmd_line_on_stack = %0*lx\n", 2*S, (unsigned long)cmd_line_on_stack);
...
// strncpy(src, dst, sizeof(src) - 1); //skall använda strlcpy men det går inte :)
//strlcpy(command_line,(const char*)stack_top,line_size);
//  strlcpy(cmd_line_on_stack, command_line, cmdl_size);
  strncpy(cmd_line_on_stack, command_line, line_size);
// command_line = malloc(line_size);
// size_t n = strlcpy(cmd_line_on_stack, command_line, line_size);
//printf("copied bytes: %s",n);
